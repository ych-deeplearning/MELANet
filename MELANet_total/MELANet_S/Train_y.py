...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, (images, gts, edges) in enumerate(train_loader, start=1):
            ...
            ual_loss = cal_ual(seg_logits=preds[5], seg_gts=gts)
            ual_loss *= ual_coef

            loss_init = structure_loss(preds[0], gts) * 0.03125 + structure_loss(preds[1], gts) * 0.0625 + structure_loss(preds[2], gts) * 0.125 + structure_loss(
                preds[3], gts) * 0.25 + structure_loss(preds[4], gts) * 0.5
            
            loss_edge = dice_loss(torch.sigmoid(preds[3]), edges)*0.125 + dice_loss(torch.sigmoid(preds[4]), edges)*0.25 + \
                        dice_loss(torch.sigmoid(preds[5]), edges)*0.5
            

            loss_final = structure_loss(preds[5], gts)
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} Loss1: {:.4f} Loss2: {:0.4f}'.
                      format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss_init.data,
                             loss_final.data))  # loss_edge.data
                logging.info(
...
def val(test_loader, model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            res = F.interpolate(result[5], size=gt.shape, mode='bilinear', align_corners=False)
            res = res.sigmoid().data.cpu().numpy().squeeze()
            res = (res - res.min()) / (res.max() - res.min() + 1e-8)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='/home/dell/User_student/ych/TrainDataset (2)/',
                        help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='/home/dell/User_student/ych/TestDataset/CAMO/',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str, default='checkpoints', help='the path to save model and log')
    ...
    # build the model
    device_ids = [0]  # if you want to use more gpus than 2, you shoule change it just like when use opt.gpu_id='1,2,6,8' , device_ids = [0,1,2,3]
    model = MELANetModel()
